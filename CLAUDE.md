...
- **Vista read-only** (`PredictionView`, Fase 4C): cuando enviaste o cerró el plazo, en vez del wizard se muestra el pronóstico completo (reusada también en Comunidad). Los clasificados se ven como un **embudo HORIZONTAL** (`BracketFunnel`): una columna por fase que se angosta (Eliminatorias de 32 → Octavos → Cuartos → Semis → Final → Campeón), con **título arriba** y **recuadro verde claro** (`bg-primary/10`, el verde de marca) por fase, y los equipos como **bandera + código** (MEX, RSA…) para ahorrar espacio; **scroll horizontal** si no caben. Podio integrado (finalistas + campeón con acento ámbar); 3er lugar + marcador final + goleador en una línea-resumen. No son llaves por partido (el modelo es "quién pasa", no cruces)
