...
- **Vercel "Fluid Active CPU"** (plan Hobby: **4 h/ciclo de facturación**): mide el **CPU activo** de las funciones server-side (RSC, Server Actions, middleware), no las invocaciones. El 24/jun/2026 iba en **2h15m/4h** y subiendo (Mundial + ~3 semanas por delante). Causa principal: el `AutoRefresh` de Comunidad re-ejecutaba el render server-side cada 60s 24/7 por cada usuario con la pestaña abierta (y #119 encareció ese render con más queries). Mitigación (PR #120): **auto-refresco consciente de actividad** — 60s solo en la ventana de un partido, 10 min en reposo (~75% menos refrescos; ver sección Comunidad). Si volviera a apretar, la siguiente palanca es cachear las lecturas **inmutables post-lock** (pronósticos, equipos) con `unstable_cache` para abaratar cada render — en la nevera hasta medir el efecto del PR #120.
