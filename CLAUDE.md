...
- **Aciertos en vivo** (post-resultados, PR #75): cada pronóstico se marca vs el marcador oficial (`✓ Exacto` / `≈ Resultado` / `✗ Falló`) cuando el partido está `final`; badge **"🎯 único exacto"** cuando alguien clavó el marcador y NADIE más lo había puesto (`exactPredictors === 1` — la versión pre-resultado "marcador único" se descartó: con ~25 marcadores plausibles y 40 jugadores habría ~10 únicos por partido, ruido); **"Tabla del día"** (puntos del día 5/2 + exactos + badge **"Pleno"**) — **colapsable, cerrada por defecto** (PR #119, mismo patrón del CTA de participantes: cabecera con avatares del top + cuerpo animado con scroll interno), para darle protagonismo a Clasificados; resalta **tu fila ("Tú")**. UX: día por defecto = **hoy**, participantes en **grid con scroll**, tabs de fecha **auto-centradas** (`useCenterActiveTab`), partidos en **acordeón** (uno abierto a la vez)
- **Eliminatorias en Comunidad** (PR #119): la lista por día incluye los días de eliminatoria (R32..3er lugar) con los marcadores de todos por cruce + consenso + tabla del día + aciertos + reacciones, **reutilizando la misma UI** (`CommunityScore` y la lógica son agnósticas al stage; `verdictPoints(v, stage)`). Un cruce solo aparece **cuando arranca** (`hasKnockoutStarted` = `live`/`final` o kickoff pasado) — que es cuando sus pronósticos se vuelven públicos por la RLS `knockout_match_started`; la **final no entra** (no usa `prediction_knockout_scores`; su marcador vive en el podio/Top 5 y el embudo). `page.tsx` trae **todos** los partidos + `prediction_knockout_scores` y `prediction_bracket` (con `fetchAll`). **Sin SQL nuevo** (las RLS existentes dan la visibilidad)
- **Sección "Clasificados"** (`QualifiedSection`, PR #119): entre la Tabla del día y el Top 5. Por cada ronda con clasificados reales (`deriveOfficialResults().advancers`, la misma fuente del scoring) una fila de banderas (scroll horizontal); tap → **modal de electores** (quiénes la eligieron + `+pts c/u`). CTA **"Ver estadísticas"** → modal con el **% de la polla** por equipo y fase (32/16/8/4); tap en un % → electores (con un **"← Estadísticas"** para volver). Se autogestiona: sin clasificados ni bracket, no se muestra
- `/comunidad/[userId]`: pronóstico completo de cualquiera (reusa `PredictionView` → embudo) — incluye sección **"Marcadores de eliminatoria"** (PR #119, solo cruces ya arrancados; `PredictionView` recibe `knockoutScores`/`knockoutMatches` opcionales)
- **Dato curioso del día** (`src/lib/daily-facts.ts` + `DailyFactCapsule`, cliente): cápsula al tope de Comunidad con 1 de 40 datos verificados del Mundial ("Día N de 40", anclado al lock, corte de día en TZ Bogotá). Con **flechas para repasar días anteriores** — nunca futuros: el server manda solo los datos del día 1 al actual (`getDailyFactsUpTo`, los futuros no viajan al cliente) y la flecha → se apaga en el del día. El índice es estado local sin persistencia: al entrar siempre se muestra el dato del día. Se retira sola pasado el día 40. Helper testeado (`daily-facts.test.ts`)
- **Auto-refresco** (`AutoRefresh` en `components/app`, montado en `CommunityView`): `router.refresh()` sin recarga visible mientras la pestaña está visible → refleja lo que carga el admin (estado de partidos, marcadores, aciertos) sin recargar (`revalidatePath` invalida el caché del server pero no empuja a los clientes abiertos). **Consciente de actividad** (PRs #120/#124/#126/#127, para ahorrar Fluid Active CPU — ver Cuidados del free tier): **5 min** dentro de la ventana de un partido (~30 min antes → 4 h después del kickoff, o si hay alguno `live`; era 60s, se subió a 5 min el 1/jul porque la ventana cubre casi todo el día con partidos y el 60s se comía el presupuesto) y **10 min** en reposo, y en reposo **solo entre 11am–11pm Bogotá** (#126; de madrugada el timer sigue vivo pero no refresca). **Catch-up al volver a la pestaña** (#124, `visibilitychange`): refresco puntual inmediato al volver → el tick lento casi no se siente. Cada refresco re-evalúa la ventana en el servidor (`hasActivity` desde los partidos + hora del server). Props `intervalMs`/`active`/`idleIntervalMs`/`activeStartHour`/`activeEndHour`. Polling deliberado en vez de Realtime. Reutilizable en otras vistas.
...
- ✅ **Comunidad: eliminatorias + Clasificados/estadísticas** — hecho (PR #119): días de eliminatoria con los marcadores de todos (consenso, tabla del día, aciertos), sección **Clasificados** por ronda (banderas → electores) con CTA **"Ver estadísticas"** (% por equipo/fase), y **"Marcadores de eliminatoria"** en el pronóstico individual. La Tabla del día quedó **colapsable**. Sin SQL nuevo (ver sección Comunidad)
...
- **`matches`** — `id`, `match_number` (1-104), `stage` (`group|r32|r16|qf|sf|3rd|final`), `group_code`, `home_team_code`/`away_team_code` (nullable), `bracket_source_home`/`away`, `kicks_off_at`, `venue`, `home_score`/`away_score`, **`winner_code`** (FK teams, nullable — ganador declarado por el admin para final/3er lugar cuando el 90' fue empate y se definió por penales; null = inferir del marcador), `status` (`scheduled|live|final|suspended` — **suspended** se pinta ámbar en calendario y Comunidad; no puntúa, solo `final` cuenta). Lectura pública; UPDATE solo admin (RLS `is_admin()`).
...
- **`prediction_reactions`** — PK (`reactor_id`, `target_user_id`, `match_id`), `reaction` ∈ (`like`,`laugh`,`fire`,`shock`,`dislike`,`eyes`,`clap`,`love`) — 8 emojis 👍👎😂🔥😱👀👏❤️ (`REACTIONS` en `comunidad/shared.ts` debe coincidir con el CHECK), `check (reactor_id <> target_user_id)`.
...
- **Vercel "Fluid Active CPU"** (plan Hobby: **4 h/ciclo de facturación**): mide el **CPU activo** de las funciones server-side (RSC, Server Actions, middleware), no las invocaciones. El 24/jun/2026 iba en **2h15m/4h**; el 1/jul en **2h56m/4h** a ~5-7 min/día (quedaban ~64 min para ~18 días → no alcanzaba). Causa principal: el `AutoRefresh` de Comunidad re-ejecuta el render server-side completo por cada usuario con la pestaña abierta, y durante el torneo la "ventana de actividad" cubre casi todo el día (partidos ~12pm-11pm) → el modo rápido corría todo el día; por eso #120 (60s activo/10 min reposo) y #126 (reposo solo 11am-11pm Bogotá) recortaron poco la curva diaria. Mitigación adicional (PR #127, 1/jul): **intervalo activo 60s → 5 min** (~80% menos refrescos en la franja dominante; el catch-up de #124 al volver a la pestaña mantiene la sensación de frescura). El breakdown por ruta del 1/jul (~118s/día) mostró además que `/ranking` + `/home` suman ~27% recomputando `loadRanking()` completo en cada visita — cachearlo (`unstable_cache` + `revalidateTag` en las actions del admin, con salvaguarda anti-RLS-anon-vacío) es la siguiente palanca diseñada, en la nevera; `/pronosticos` (~23%) son autosaves de usuarios (trabajo real, no tocar). **En la nevera, en orden**: (1) la palanca de fondo "versión-gate" — función SQL `community_version()` (hash de status/marcadores/equipos/ganador + conteo de partidos arrancados + goleador) consultada por el navegador **directo a Supabase** (0 CPU en Vercel) y `router.refresh()` solo si la versión cambió (~90-95% de ahorro; diseñada el 1/jul, no implementada porque el usuario prefirió la opción simple); (2) cachear lecturas inmutables post-lock con `unstable_cache`. Válvula de emergencia si se roza el tope: apagar `AutoRefresh` (1 línea) — con el catch-up de #124 la UX queda aceptable.
