...
- **Auto-refresco** (`AutoRefresh` en `components/app`, montado en `CommunityView`): `router.refresh()` cada 60s mientras la pestaña está visible (se pausa al ocultarla) → el estado de los partidos, marcadores y aciertos que carga el admin se reflejan sin recargar (`revalidatePath` invalida el caché del server pero no empuja a los clientes abiertos). Polling deliberado en vez de Realtime. Reutilizable en otras vistas.
...
- **Comunidad: marcadores de eliminatoria** (planificado, ~28 jun) — extender los tabs por fecha de Comunidad para incluir los días de eliminatoria y mostrar los marcadores de todos por cruce, reutilizando la misma UI (consenso, pronósticos por partido, tabla del día, aciertos 5/2). Visibilidad por cruce ya garantizada por la RLS `knockout_match_started` (solo se abren al arrancar el cruce). Hoy Comunidad solo muestra fase de grupos
- **Comunidad: sección "Clasificados" + estadísticas** (planificado junto con lo anterior, ~26-28 jun) — entre la tabla del día y el Top 5: por cada ronda resuelta (R32→Octavos→Cuartos→Semis) una fila de banderas de los equipos que pasaron (scroll horizontal); tap en bandera → modal con los usuarios que la eligieron en esa ronda (ganaron 2/3/5/8 pts). CTA **"Ver estadísticas"** → modal con los 48 equipos y el % de la polla que dijo que pasaban a cada fase; tap en equipo → quiénes lo eligieron. Datos: `prediction_bracket` + advancers reales. Versión "por fase" en vez de bracket con líneas (responsive en móvil)
